...
public class LoginDayObject
{
	...
	public LoginDayObject(GameObject obj, int day)
	{
		...
		m_imgItem = GameObjectUtil.FindChildGameObjectComponent<UISprite>(m_clearGameObject, "img_login_item");
		...
		m_clearAnimation = obj.GetComponent<Animation>();
	}
		...
		if (m_imgItem != null && m_imgChara != null && m_imgChao != null)
		{
			if (id >= 0)
			{
				...
				case 3:
					...
					m_imgChara.alpha = 1f;
					m_imgChao.alpha = 0f;
					m_imgChara.spriteName = "ui_tex_player_set_" + CharaTypeUtil.GetCharaSpriteNameSuffix(new ServerItem((ServerItem.Id)id).charaType);
					break;
				case 4:
				{
					m_imgItem.alpha = 0f;
				...
				default:
				{
					int rewardType = (int)new ServerItem((ServerItem.Id)id).rewardType;
					m_imgItem.alpha = 1f;
					m_imgChara.alpha = 0f;
					m_imgChao.alpha = 0f;
					m_imgItem.spriteName = "ui_cmn_icon_item_" + rewardType;
					break;
				}
				}
			}
